...
Mesh* Mesh::load(string pFileName)
{
	...
	if (file.is_open()) {
		...
		cout << "vertices list size: " << mesh->_vertices.size() << endl;
		cout << "indices list size: " << mesh->_indices.size() << endl;
		return mesh;
	}
	else {
...
btTriangleMesh* Mesh::GetCollisionMesh()
{
	...
	cout << "vertices at GetCollisionMesh: " << _vertices.size() << endl;
	try
	{
		for (size_t iter = 0; iter < _indices.size(); iter += 3)
		{
			const btVector3 &a = toBtVector(_vertices[_indices[iter + 0]]);
			const btVector3 &b = toBtVector(_vertices[_indices[iter + 1]]);
			const btVector3 &c = toBtVector(_vertices[_indices[iter + 2]]);
