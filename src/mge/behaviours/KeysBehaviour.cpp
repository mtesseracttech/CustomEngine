...
void KeysBehaviour::update( float pStep )
{
	...
	if(_owner->getRigidBody() != nullptr)
	{
		...
		btVector3 appliedForce	(0, 0, 0);
		btVector3 appliedTorque	(0, 0, 0);

		if (sf::Keyboard::isKeyPressed(sf::Keyboard::Up)) {
			appliedForce	= btVector3(0, 0, 10);
		}
		...
		if (sf::Keyboard::isKeyPressed(sf::Keyboard::Left)) {
			...
			//appliedTorque	= btVector3(0, 20, 0);
		}
		...
		{
			rb->applyCentralForce(btVector3(0, 100, 0));
			std::cout << "JUMP!" << std::endl;
		}

		btVector3 correctedForce = (rbTrans * appliedForce) - rbTrans.getOrigin();
		rb->activate();
		rb->applyCentralForce(correctedForce);
		_owner->rotate(glm::radians(turnSpeed*pStep), glm::vec3(0.0f, 1.0f, 0.0f));
		//rb->applyTorque(appliedTorque);
	}
	else
	{
		if (sf::Keyboard::isKeyPressed(sf::Keyboard::Up)) {
